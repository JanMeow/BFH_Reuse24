...
class innerMaterialGenerate:
    def __init__(self, boardDB, surfaceList, materialList, thicknessList, moduleDistance=1800, wallFrame=None, claddingDirection=None, moduleCurve=None, moduleCrvDist=100):
        self.boardDB = boardDB
        ...
        for self.id, (srfList, matList) in enumerate(zip(self.surfaceList, self.materialList)):
            ...
            for entireSrf_id, entireSrf in enumerate(srfList):
                ...
                if len(self.moduleCurve) != 0:
                    srfList = self.create_module_byCurve(entireSrf, workPlane, self.moduleCurve, self.moduleCrvDist)
                else:
                    srfList = self.create_module_withDistance(entireSrf, workPlane, self.moduleDistance)
                ...
                for self.module_id, srf in enumerate(srfList):
                    ...
                    if matType == "board":
                        if mat.searchDB:
                            useLength, useWidth, useQuantity, useId = self.find_board(self.boardDB, mat.length, mat.width)
                        else:
                            useLength, useWidth = mat.length, mat.width

                        boardGeo = self.create_board(srf, workPlane, useLength, useWidth, mat.thickness, mat.direction)
    ...
    def find_board(self, searchDB, searchLength, searchWidth):
        widthDB = []
        lengthDB = []
        quantityDB = []
        for board in searchDB.Branches:
            w, l = board[0].Width, board[0].Height
            widthDB.append(w)
            lengthDB.append(l)
            quantityDB.append(board[1])

        boardList = []
        for id, (l, w, q) in enumerate(zip(lengthDB, widthDB, quantityDB)):
            idName = "board_" + str(id)
            boardList.append((w,l,q,idName))

        euclideanDistList = []
        for l, w, q, id in boardList:
            euclideanDist = abs(searchWidth-w) + abs(searchLength-l)
        ...
        useLength, useWidth, useQuantity, useId = tileData

        return (useLength, useWidth, useQuantity, useId)
...
class generateCladding:
    ...
    def __init__(self, windowDB, doorDB, claddingDB, wallGeo, windowGeo, doorGeo, claddingWidth=299, claddingLength=399, kindNum=4, wWeight=5, lWeight=4, claddingDirection=True, horizontalOverlap=0, verticalOverlap=0, horizontalAngle=0, verticalAngle=0, substructWidth=40, substructThickness=20, offsetDist=0,**kwargs):
        """
...
if init:
    ...
    if customizeTile:
        ...
        claddingObj = generateCladding(windowDB=windowData, doorDB=doorData, claddingDB=claddingData, wallGeo=wallGeo, windowGeo=windowGeo, doorGeo=doorGeo, claddingWidth=claddingWidth, claddingLength=claddingLength, kindNum=kindNum, wWeight=wWeight, lWeight=lWeight, claddingDirection = claddingDirection, horizontalOverlap=horiOverlap, verticalOverlap=vertiOverlap, horizontalAngle=horiAngle, verticalAngle=vertiAngle, substructWidth=substructWidth, substructThickness=substructThickness, offsetDist=offsetCladdingDist)
    ...
    # compensatedWindowGeo = claddingObj.compensatedWindowGeoNext


    # Calculate innerMaterial Part
    offsetted_surface = offset_brep(wallForInnerMaterial, offsetList, wallFrame)

    wallObj = innerMaterialGenerate(boardData, offsetted_surface, materialList, material_thickness, moduleDistance, wallFrame, claddingDirection, moduleCurve, moduleCrvDist)
    ...
    for key in wallObj.materialInfoDict:
        materialInfo.append(key)
        materialInfo.append(str(wallObj.materialInfoDict[key]))
        # print(key)
